...
def main_page():
    ...
    with col1:
        if st.button("주거비 지원", use_container_width=True):
            st.session_state.page = "housing"
            st.rerun()
    with col2:
        if st.button("단기 주거 매칭", use_container_width=True):
...
def short_term_page():
    st.set_page_config(page_title="단기 주거 매칭", layout="wide")
    st.title("단기 주거 매칭")
    ...
    col_select, col_money = st.columns(2)
    col_select.selectbox("희망 지역", ["서울", "경기", "부산"])
